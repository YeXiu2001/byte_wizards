  ...
  <div class="row">
    <div class="col-12">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">Whistleblower Reports Table</h3>
    ...
    @else
        <table class="table table-primary table-bordered table-striped" id="myTable">
            ...
            <tbody>
                @foreach($wblowerreps as $w)
                    <tr>
                        <td>{{ $w->date_of_incident }}</td>
                        <td>{{ $w->fname }} {{ $w->lname }}</td>
                        <td>{{ $w->contactno }}</td>
                        <td>{{ $w->email }}</td>
                        <td>{{ $w->name_accused }} ({{ $w->position }}, {{ $w->department }})</td>
                        <td>{{ $w->misconduct }}</td>
                        <td>{{ $w->further_infos }}</td>
                        <td>{{ $w->status }}</td>
                        <td> 
                                                <!-- Edit button -->
                          <button type="button" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#updateStatusModal" data-wblower-id="{{ $w->id }}">
                              <i class="fas fa-edit"></i>
                          </button>

                          <!-- Delete button with SweetAlert2 confirmation -->
                          <form id="deleteForm{{ $w->id }}" action="{{ route('deleteWblower', ['id' => $w->id]) }}" method="POST">
                              @csrf
                              @method('DELETE')
                              <button type="button" class="btn btn-sm btn-danger" onclick="confirmDelete('{{ $w->id }}')">
...
<script>
    $(document).ready(function () {
        ...
        $('#updateStatusModal').on('show.bs.modal', function (event) {
            ...
            var wblowerId = button.data('wblower-id');
            var form = $('#updateStatusForm');

            // Set the action attribute of the form dynamically
            var actionUrl = "{{ url('whistleblowerreps/update-status') }}/" + wblowerId;
...
<script>
    function confirmDelete(wblowerId) {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('deleteForm' + wblowerId).submit();
            }
        });
    }
</script>
